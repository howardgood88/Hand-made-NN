...
class Prepare_Task_Data:
	...
	def __init__(self, target_path):
		...
		self.root_dir = '/home/qiuhui/Desktop/howard/rain_CDR'
	...
	def _prepare_data(self, grid_limit, task_name):
		...
		x_target_path = os.path.join(self.target_path, task_name, 'one_hour')
		y_target_path = os.path.join(self.target_path, task_name, 'one_' + task_name)
		...
		# prepare x
		file_dir = os.path.join(self.root_dir, 'npy', task_name, 'one_hour')
		load_and_save(file_dir, x_target_path)
		# prepare y
		file_dir = os.path.join(self.root_dir, 'npy', task_name, 'one_' + task_name)
	...
	def _get_X_and_Y(self, task_name):
		...
		x_dir = os.path.join(self.target_path, task_name, 'one_hour')
		y_dir = os.path.join(self.target_path, task_name, 'one_' + task_name)
	...
	def Task_max(self, grid_limit=[(45, 60), (45, 60)], generate_data=False):
		...
		#print('------------{}----------'.format(X_array.shape))
		#X_array_date = X_array
		...
		#				date = X_array_date[i, j, row, col, 1]
		#				print("---------------------------{}-------------------------".format(int(date)))
	...
	def Task_max_min_avg(self, grid_limit=[(45, 60), (45, 60)], generate_data=False):
		...
		if generate_data:
			x_target_path = os.path.join(self.target_path, task_name, 'one_hour')
			y_target_path = os.path.join(self.target_path, task_name, 'one_hour_min_avg_max')
			if not os.path.exists(x_target_path):
				os.makedirs(x_target_path)
			if not os.path.exists(y_target_path):
				os.makedirs(y_target_path)
				...
				for j in range(max_Y.shape[1]):
					for row in range(max_Y.shape[2]):
						for col in range(max_Y.shape[3]):
							...
							min_avg_max_Y[i, j, row, col, 0] = min_Y[i, j, row, col, 0]  # quadrant_id
							min_avg_max_Y[i, j, row, col, 1] = min_Y[i, j, row, col, 1]  # timestamp

							min_avg_max_Y[i, j, row, col, 2] = min_Y[i, j, row, col, -1]  # tweet_num
							min_avg_max_Y[i, j, row, col, 3] = avg_Y[i, j, row, col, -1]  # tweet_num
							min_avg_max_Y[i, j, row, col, 4] = max_Y[i, j, row, col, -1]  # tweet_num
