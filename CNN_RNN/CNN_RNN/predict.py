...
root_dir = '/home/qiuhui/Desktop/howard/'
scalar = None
...
def predict_MTL_train(neural, X_array, Y_array, _X_array, _Y_array, model_path):
    print(X_array.shape)
    ...
    new_Y_array = un_feature_scaling(_Y_array[:, :, :, :, 2:], scaler)
    _Y_array = copy(_Y_array, new_Y_array)

    # grid_id, timestamp, real_min, real_avg, real_max, prediciton_min, prediction_avg, prediction_max
    Y_real_prediction = np.concatenate((_Y_array, predict_y), axis=-1)
    du.save_array(Y_real_prediction, './result/Y_real_prediction.npy')

    print('-' * 20, 'task min:', '-' * 20)
    compute_loss_rate(_Y_array[:, :, :, :, 2, np.newaxis],
                      predict_y[:, :, :, :, 0, np.newaxis])
    plot_predict_vs_real(_Y_array[:, :, :, :, (0, 1, 2)], predict_y[
                         :, :, :, :, 0, np.newaxis])
    print('-' * 30)
    print('-' * 20, 'task avg:', '-' * 20)
    compute_loss_rate(_Y_array[:, :, :, :, 3, np.newaxis],
                      predict_y[:, :, :, :, 1, np.newaxis])
    plot_predict_vs_real(_Y_array[:, :, :, :, (0, 1, 3)], predict_y[
                         :, :, :, :, 1, np.newaxis])
    print('-' * 30)
    print('-' * 20, 'task max:', '-' * 20)
    compute_loss_rate(_Y_array[:, :, :, :, 4, np.newaxis],
                      predict_y[:, :, :, :, 2, np.newaxis])
    plot_predict_vs_real(_Y_array[:, :, :, :, (0, 1, 4)], predict_y[
                         :, :, :, :, 2, np.newaxis])
    print('-' * 30)
...
TK2 = Prepare_Task_Data_2('/home/qiuhui/Desktop/bo/predict_telecom_traffic/CNN_RNN/npy/final')
X_array, Y_array = TK.Task_max_min_avg(generate_data=False)
_X_array, _Y_array = TK2.Task_max_min_avg(generate_data = False)
Y_array = Y_array[:, :, 10:13, 10:13, :]
_Y_array = _Y_array[:, :, 10:13, 10:13, :]

X_array_date = X_array[:, :, :, :, 0, np.newaxis]
X_array_date = time_feature(X_array_date)
X_array_date, _ = feature_scaling(X_array_date, feature_range=(10, 70))
_X_array_date = _X_array[:, :, :, :, 0, np.newaxis]
...
_new_X_array, _scaler = feature_scaling(_X_array[:, :, :, :, 2:])
_new_Y_array, _ = feature_scaling(_Y_array[:, :, :, :, 2:], _scaler)

X_array = copy(X_array, new_X_array)
Y_array = copy(Y_array, new_Y_array)
_X_array = copy(_X_array, _new_X_array)
_Y_array = copy(_Y_array, _new_Y_array)

del new_X_array, new_Y_array, _new_X_array, _new_Y_array
...
_X_array_train = _X_array[0:batch_size]   # should correspond to bathc size
_X_array_train_date = _X_array_date[0:batch_size]
_X_array_train = np.concatenate((_X_array_train_date, _X_array_train), axis=-1)
_Y_array_train = _Y_array[0:batch_size]  # should correspond to bathc size
...
_X_array_test = _X_array[_X_array.shape[0] - batch_size:, :, :, :, -1, np.newaxis]
_X_array_test_date = _X_array_date[_X_array.shape[0] - batch_size:]
_X_array_test = np.concatenate((_X_array_test_date, _X_array_test), axis=-1)
_Y_array_test = _Y_array[_Y_array.shape[0] - batch_size:]
...
model_path = {
    ...
    'save_path': '/home/qiuhui/Desktop/howard/CNN_RNN/output_model/CNN_RNN.ckpt'
}

# predict_train(cnn_rnn, X_array_train, Y_array_train, model_path)
# predict_train(cnn_rnn, X_array_test, Y_array_test, model_path)
neural.create_MTL_task(X_array_test, Y_array_test[:, :, :, :, 2, np.newaxis], 'rain_min_traffic')
neural.create_MTL_task(X_array_test, Y_array_test[:, :, :, :, 3, np.newaxis], 'rain_avg_traffic')
neural.create_MTL_task(X_array_test, Y_array_test[:, :, :, :, 4, np.newaxis], 'rain_max_traffic')
neural.create_MTL_task_2(_X_array_test, _Y_array_test[:, :, :, :, 2, np.newaxis], 'CDR_min_traffic')
neural.create_MTL_task_2(_X_array_test, _Y_array_test[:, :, :, :, 3, np.newaxis], 'CDR_avg_traffic')
neural.create_MTL_task_2(_X_array_test, _Y_array_test[:, :, :, :, 4, np.newaxis], 'CDR_max_traffic')
predict_MTL_train(neural, X_array_test, Y_array_test, _X_array_test, _Y_array_test, model_path)
