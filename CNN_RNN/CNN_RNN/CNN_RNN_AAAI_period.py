...
class Multitask_Neural_Network(Tf_Utility):
	...
	def build_MTL(self, input_data_shape, output_data_shape):
		...
		self.output_channel = output_data_shape[3]
	...
	def build_MTL_period(self, input_data_shape, output_data_shape):
		...
		self.Ys_period = tf.placeholder(tf.float32, shape=[
			None, self.output_temporal_period, self.output_vertical_period, self.output_horizontal_period, 1], name='Input_y')
	...
	def build_MTL_trend(self, input_data_shape, output_data_shape):
		...
		self.Ys_trend = tf.placeholder(tf.float32, shape=[
			None, self.output_temporal_trend, self.output_vertical_trend, self.output_horizontal_trend, 1], name='Input_y')
	...
	def trainable_var_fusion(self, N1, N2, N3, N4):
		with tf.variable_scope('fusion'):
			...
			net_4 = tl.layers.Parametric_Matrix_Fusion_Layer(N4, name = 'fusion_layer_mul_4')
			self.tl_share_output = tl.layers.ElementwiseLayer([net_1, net_2, net_3, net_4], combine_fn = tf.add, name = 'fusion_layer_add')
	...
	def _MTL_testing_data(self, sess, test_x_1, test_y_1, test_x_2, test_y_2
		, test_x_3, test_y_3, test_x_4, test_y_4, task_name):
		...
		for batch_index in range(batch_num):
			...
			batch_x_3 = test_x_3[batch_index * self.batch_size: (batch_index + 1) * self.batch_size]
			batch_y_3 = test_y_3[batch_index * self.batch_size: (batch_index + 1) * self.batch_size]
			batch_x_4 = test_x_4[batch_index * self.batch_size: (batch_index + 1) * self.batch_size]
			batch_y_4 = test_y_4[batch_index * self.batch_size: (batch_index + 1) * self.batch_size]
			...
			feed_dict = {
				...
				self.add_noise_2: 1,
				self.Xs_period: batch_x_3,
				self.Ys_period: batch_y_3,
				self.keep_prob_period: 0.85,
				self.add_noise_period: 1,
				self.Xs_trend: batch_x_4,
				self.Ys_trend: batch_y_4,
	...
	def run_multi_task_2(self, sess, task_name_1='', task_name_2='', task_name_3='', task_name_4='', optimizer='optomizer'):
		def run_task_optimizer(sess, batch_x_1, batch_y_1, batch_x_2, batch_y_2
			, batch_x_3, batch_y_3, batch_x_4, batch_y_4, task_name, optimizer='optomizer'):
			task_dic = self.multi_task_dic[task_name]
			list1 = np.full((100, 512), 2)
			feed_dict = {
				self.Xs: batch_x_1,
		...
		task = self.multi_task_dic[task_name_4]
		training_batch_op = task['shuffle_batch_OP']
		index_4, batch_x_4, batch_y_4 = sess.run(training_batch_op)
		...
		cost, L2 = run_task_optimizer(sess, batch_x_1, batch_y_1, batch_x_2, batch_y_2
			, batch_x_3, batch_y_3, batch_x_4, batch_y_4, task_name_1)
		task['training_temp_loss'] += cost
	...
	def run_task_evaluate(self, sess, fig, epoch, display_step=50, task_name_1='', task_name_2='',
		task_name_3='', task_name_4=''):
			...
			task = self.multi_task_dic[task_name_3]
			index_3, testing_X_3, testing_Y_3 = self.read_all_data_from_Tfreoced_period_trend(
				task['testing_file'],
				self.input_temporal_period,
				self.input_vertical_period,
				self.input_horizontal_period,
				...
				self.Y_horizontal_period,
				self.Y_channel_period)
			index_3, batch_x_sample_3, batch_y_sample_3 = sess.run(task['batch_OP'])

			task = self.multi_task_dic[task_name_4]
			index_4, testing_X_4, testing_Y_4 = self.read_all_data_from_Tfreoced_period_trend(
				task['testing_file'],
				self.input_temporal_trend,
				self.input_vertical_trend,
				self.input_horizontal_trend,
				...
				self.Y_horizontal_trend,
				self.Y_channel_trend)
			index_4, batch_x_sample_4, batch_y_sample_4 = sess.run(task['batch_OP'])

			task = self.multi_task_dic[task_name_2]
			index_2, testing_X_2, testing_Y_2 = self.read_all_data_from_Tfreoced_2(
				task['testing_file'],
				self.input_temporal,
				self.input_vertical,
				self.input_horizontal,
			...
			testing_loss, testing_accu, prediction = self._MTL_testing_data(sess, testing_X_1, testing_Y_1
				, testing_X_2, testing_Y_2, testing_X_3, testing_Y_3
				, testing_X_4, testing_Y_4, task_name_2)
			training_loss_nodrop, training_accu, train_prediction = self._MTL_testing_data(sess, batch_x_sample_1, batch_y_sample_1
				, batch_x_sample_2, batch_y_sample_2, batch_x_sample_3, batch_y_sample_3
				, batch_x_sample_4, batch_y_sample_4, task_name_2)
			...
			with tf.device('/gpu:0'):
				try:
					for epoch in range(self.iter_epoch):
						self.run_multi_task_2(sess, 'CDR_max_traffic', 'rain_traffic', 'period', 'trend')
						self.run_multi_task_2(sess, 'CDR_avg_traffic', 'rain_traffic', 'period', 'trend')
						self.run_multi_task_2(sess, 'CDR_min_traffic', 'rain_traffic', 'period', 'trend')
						# run_multi_task(sess, '10_mins')

						if epoch % display_step == 0 and epoch is not 0:
							self.run_task_evaluate(sess, max_fig, epoch, task_name_1='rain_traffic', task_name_2='CDR_max_traffic',
								 task_name_3='period', task_name_4='trend')
							self.run_task_evaluate(sess, avg_fig, epoch, task_name_1='rain_traffic', task_name_2='CDR_avg_traffic',
								 task_name_3='period', task_name_4='trend')
							self.run_task_evaluate(sess, min_fig, epoch, task_name_1='rain_traffic', task_name_2='CDR_min_traffic',
								 task_name_3='period', task_name_4='trend')
...
class CNN_RNN_2_test(Multitask_Neural_Network):
	def __init__(self, input_data_shape_2, output_data_shape_2, input_data_shape_period,
			output_data_shape_period, input_data_shape_trend, output_data_shape_trend,
			input_data_shape, output_data_shape, config):
		...
		network_2 = self.external_FC(input_data_shape, output_data_shape, config)

		if config.trainable_w_fn == True:
			self.trainable_var_fusion(network_1, network_2, network_3, network_4)
		else:
