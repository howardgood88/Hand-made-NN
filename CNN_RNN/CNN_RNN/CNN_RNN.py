...
class Multitask_Neural_Network(Tf_Utility):
	...
	def build_MTL_2(self, input_data_shape, output_data_shape):
		...
		self.Xs_2 = tf.placeholder(tf.float32, shape=[
			None, self.input_temporal, self.input_vertical, self.input_horizontal, self.input_channel], name='Input_x_2')
		self.Ys_2 = tf.placeholder(tf.float32, shape=[
			None, self.output_temporal, self.output_vertical, self.output_horizontal, 1], name='Input_y_2')
	...
	def build_flatten_layer_2(self, tl_input):

		flat_tl = tl.layers.FlattenLayer(tl_input, name='flatten_layer_2')
		network = tl.layers.DenseLayer(flat_tl, W_init=self.fully_connected_W_init, n_units=self.fully_connected_units, act=lambda x: tl.act.lrelu(x, 0.2), name='fully_connect_1_2')
		network = tl.layers.DropoutLayer(network, keep=self.keep_rate, name='drop_1_2')
		return network

	def trainable_var_fusion(self, N1, N2):
		with tf.variable_scope('fusion'):
			...
			self.tl_share_output = tl.layers.ElementwiseLayer([net_1, net_2], combine_fn = tf.add, name = 'fusion_layer_add')

	def concat_fusion(self, N1, N2):
		with tf.variable_scope('fusion'):
			network = tl.layers.ConcatLayer([N1, N2], name = 'concat_fusion_layer')
			self.tl_share_output = tl.layers.DropconnectDenseLayer(network, keep = self.keep_rate
				, n_units = self.fully_connected_units, W_init = self.fully_connected_W_init
				, act=lambda x: tl.act.lrelu(x, 0.2), name='fully_connect_concat_fusion')
	...
	def create_MTL_task_2(self, input_x, input_y, task_name, loss_type='MSE'):
		self.multi_task_dic[task_name] = self.__create_MTL_output(self.tl_share_output, self.Ys_2, task_name, loss_type='MSE')
		self.__set_training_data(input_x, input_y, task_name)
		self.saver = tf.train.Saver()
	...
	def __summarized_report(self):
		...
		for key in task_keys:
			if key == 'SP_min_traffic' or 'SP_avg_traffic' or 'SP_max_traffic':
				continue
			task_summ_dict = {}
	...
	def _MTL_testing_data(self, sess, test_x_1, test_y_1, test_x_2, test_y_2, task_name):
		...
		for batch_index in range(batch_num):
			...
			batch_x_2 = test_x_2[batch_index * self.batch_size: (batch_index + 1) * self.batch_size]
			batch_y_2 = test_y_2[batch_index * self.batch_size: (batch_index + 1) * self.batch_size]
	...
	def run_multi_task_2(self, sess, task_name_1='', task_name_2='', optimizer='optomizer'):
		def run_task_optimizer(sess, batch_x_1, batch_y_1, batch_x_2, batch_y_2, task_name, optimizer='optomizer'):
			task_dic = self.multi_task_dic[task_name]
			list1 = np.full((100, 512), 2)
			feed_dict = {
				self.Xs: batch_x_1,
		...
		# batch_x, batch_y = get_multi_task_batch(batch_x, batch_y)
		cost, L2 = run_task_optimizer(sess, batch_x_1, batch_y_1, batch_x_2, batch_y_2, task_name_2)
		task['training_temp_loss'] += cost

	def run_task_evaluate(self, sess, fig, epoch, display_step=50, task_name_1='', task_name_2=''):
			task = self.multi_task_dic[task_name_1]
			index_1, testing_X_1, testing_Y_1 = self.read_all_data_from_Tfreoced(
				task['testing_file'],
				self.input_temporal,
				self.input_vertical,
				self.input_horizontal,
			...
			task = self.multi_task_dic[task_name_2]
			index_2, testing_X_2, testing_Y_2 = self.read_all_data_from_Tfreoced(
				task['testing_file'],
				self.input_temporal,
				self.input_vertical,
				self.input_horizontal,
				...
				self.Y_horizontal,
				self.Y_channel)
			index_2, batch_x_sample_2, batch_y_sample_2 = sess.run(task['batch_OP'])
			# batch_x_sample, batch_y_sample = get_multi_task_batch(batch_x_sample, batch_y_sample)
			# testing_X, testing_Y = get_multi_task_batch(testing_X, testing_Y)
			testing_loss, testing_accu, prediction = self._MTL_testing_data(sess, testing_X_1, testing_Y_1
				, testing_X_2, testing_Y_2, task_name_2)
			training_loss_nodrop, training_accu, train_prediction = self._MTL_testing_data(sess, batch_x_sample_1, batch_y_sample_1
				, batch_x_sample_2, batch_y_sample_2, task_name_2)

			task['training_temp_loss'] /= display_step

			self.multi_task_dic[task_name_2]['testing_MSE_history'].append(testing_loss)
			self.multi_task_dic[task_name_2]['training_MSE_history'].append(training_loss_nodrop)
			self.multi_task_dic[task_name_2]['testing_accurcy_history'].append(testing_accu)
			self.multi_task_dic[task_name_2]['training_accurcy_history'].append(training_accu)
			...
			self._plot_predict_vs_real(
				fig,
				task_name_2,
				testing_Y_2[:100, 0, 0, 0, 0],
				prediction[:100, 0, 0, 0, 0],
				batch_y_sample_2[:100, 0, 0, 0, 0],
			...
			with tf.device('/gpu:0'):
				try:
					for epoch in range(self.iter_epoch):
						self.run_multi_task_2(sess, 'SP_max_traffic', 'CDR_max_traffic')
						self.run_multi_task_2(sess, 'SP_avg_traffic', 'CDR_avg_traffic')
						self.run_multi_task_2(sess, 'SP_min_traffic', 'CDR_min_traffic')
						# run_multi_task(sess, '10_mins')

						if epoch % display_step == 0 and epoch is not 0:
							self.run_task_evaluate(sess, max_fig, epoch, task_name_1='SP_max_traffic', task_name_2='CDR_max_traffic')
							self.run_task_evaluate(sess, avg_fig, epoch, task_name_1='SP_avg_traffic', task_name_2='CDR_avg_traffic')
							self.run_task_evaluate(sess, min_fig, epoch, task_name_1='SP_min_traffic', task_name_2='CDR_min_traffic')
...
class CNN_RNN_2_test(Multitask_Neural_Network):
	def __init__(self, input_data_shape, output_data_shape, config):
		...
		network_1 = self.CNN_RNN_1(input_data_shape, output_data_shape, config)

		network_2 = self.CNN_RNN_2(input_data_shape, output_data_shape, config)

		if config.trainable_w_fn == True:
			self.trainable_var_fusion(network_1, network_2)
		else:
			self.concat_fusion(network_1, network_2)
		#self.tl_share_output.print_layers()
	...
	def CNN_RNN_2(self, input_data_shape, output_data_shape, config):
		self.__parse_config(config)
		self.build_MTL_2(input_data_shape, output_data_shape)
		if self.STPP == True:
			tl_output = self.__build_CNN_RNN_STPP_2(self.Xs_2)
		else:
			tl_output = self.__build_CNN_RNN_2(self.Xs_2)
		return self.build_flatten_layer_2(tl_output)
	...
	def __build_CNN_RNN_STPP_2(self, Xs):
		def build_CNN_network(input_X, is_training=1):
			with tf.variable_scope('CNN_2'):
				...
				network = tl.layers.BatchNormLayer(network, is_train=is_training, name='batchnorm_layer_2')
				network_5x5 = tl.layers.Conv2dLayer(
					network,
					act=self.CNN_layer_activation_fn,
					W_init=self.CNN_layer_1_5x5_conv_Winit,
				...
				STPP_network_1 = tl.layers.FlattenLayer(STPP_network_1, name='STPP_flatten_4x4_2')
				...
				STPP_network_2 = tl.layers.FlattenLayer(STPP_network_2, name='STPP_flatten_2x2_2')
				network = tl.layers.ConcatLayer([STPP_network_1, STPP_network_2], name = 'STPP_concat_layer_2')
				network = tl.layers.DropoutLayer(network, keep=self.keep_rate, name='drop_2')
				# network = tl.layers.FlattenLayer(network, name='flatten_layer')
		...
		def build_bi_RNN_network(input_X, is_training=1):
			def make_gaussan_state_initial(scope_name, stddev=self.RNN_init_state_noise_stddev):
				with tf.variable_scope(scope_name):
					init_state = tf.Variable(np.zeros((self.RNN_num_layers, 2, self.batch_size, self.RNN_hidden_node_size), dtype=np.float32), trainable=True)
					result_intital_state = tf.cond(self.add_noise_2, lambda: init_state + tf.random_normal(tf.shape(init_state), stddev=stddev), lambda: init_state)
			...
			with tf.variable_scope('BI_RNN_2'):
				...
				network = tl.layers.ReshapeLayer(input_X, shape=[-1, self.RNN_num_step, int(input_X.outputs._shape[-1])], name='reshape_layer_1_2')
				print('RNN_one_input shape:{}'.format(network))
				network = tl.layers.BiRNNLayer(
					network,
					cell_fn=self.RNN_cell,
					cell_init_args=self.RNN_cell_init_args,
		...
		with tf.variable_scope('CNN_RNN_2'):
			...
			network = tl.layers.BatchNormLayer(tl_CNN_output, is_train=False, name='batchnorm_layer_1_2')
