...
from CNN_RNN_period_stack import CNN_RNN_2_test
import os
...
def make_period_data(X_array, p = 24, lp = 6):
	...
	for i in range(24 * lp): # 24 * 6, do this cuz the former 143 hours has no 6 days ago data,
		for j in range(lp): # so I make them fill in the fixed bigining 6 days data as the same
			...
			period[i, :, :, :, :] = X_array[(i % p) + j * p, :, :, :, :]
	for i in range(X_array.shape[0] - 24 * lp): # 1338 - 24 * 6
		for j in range(lp):
			period[i + 24 * lp, :, :, :, :] = X_array[i + j * p, :, :, :, :]
...
def make_trend_data(X_array, q = 168, lq = 6):
	...
	for i in range(24 * 7 * lq): # 24 * 7 * 6
		for j in range(lq):
			...
			trend[i, :, :, :, :] = X_array[(i % q) + j * q, :, :, :, :]
	for i in range(X_array.shape[0] - 24 * 7 * lq): # 1338 - (24 * 7 * 6)
		for j in range(lq):
			trend[i + 24 * 7 * lq, :, :, :, :] = X_array[i + j * q, :, :, :, :]
...
def train():
	...
	period = make_period_data(_X_array[:, :, :, :, -1, np.newaxis], p = 24, lp = 1) # p: 1 day ago, lp: take 6 days data
	period, _ = feature_scaling(period, feature_range = (0.1, 255))
	trend = make_trend_data(_X_array[:, :, :, :, -1, np.newaxis], q = 168, lq = 1) # q: 1 week ago, lq: take 6 weeks data
	...
	_X_array = np.concatenate((_X_array, period, trend), -1)
	...
	neural = CNN_RNN_2_test(input_data_shape_2, output_data_shape_2, hyper_config)
