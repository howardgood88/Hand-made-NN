...
def train():
	...
	with open(path, newline='') as csvfile:
		...
		for row in rows:
			temporature.append(float(row[2]))
	...
	for ele in wind_speed:
		if ele > wind_speed_avg:
			wind_speed_list.append(1)
		else:
			wind_speed_list.append(0)
	'''
	...
	'''
	for ele in pollution:
		if ele > pollution_AQI_threshold:
			pollution_list.append(1)
		else:
			pollution_list.append(0)
	'''
	...
	period = make_period_data(_X_array, p = 24, lp = 6) # p: 1 day ago, lp: take 6 days data
	trend = make_trend_data(_X_array, q = 168, lq = 6) # q: 1 week ago, lq: take 6 weeks data
	...
	neural = CNN_RNN_2_test(input_data_shape, output_data_shape, input_data_shape_2, output_data_shape_2, hyper_config)
