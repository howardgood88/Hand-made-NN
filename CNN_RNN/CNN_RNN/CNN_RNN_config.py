...
class HyperParameterConfig:
	def __init__(self):
		self.iter_epoch = 2500
		self.batch_size = 100
		self.learning_rate = 0.0003# 0.0014 rain(CVV_RNN):0.0002, period:0.0008 0.000008
		print('learning rate is: {}'.format(self.learning_rate))
		self.keep_rate = 0.85 # 0.85
		self.weight_decay = 0.001  # 0.001
		self.combine_fn = tf.add #avg should type 'avg'
		self.trainable_w_fn = True
		self.STPP = True
		self.embedded = 100
	...
	def CNN_RNN(self):
		...
		self.CNN_layer_2_STPP_Ps_1 = 4
		self.CNN_layer_2_STPP_Ps_2 = 2
		self.CNN_layer_2_pooling = 'avg_pool'

	def CNN_RNN_two(self):
		self.RNN_num_layers = 3
		self.RNN_num_step = 6
		self.RNN_hidden_node_size = 256
		self.RNN_cell = 'LSTMcell'
